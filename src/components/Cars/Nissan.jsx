...
export function Nissan({ rigidBody, onSpeedChange, ...props }) {
  ...
  const FORCE = boostActive ? 13 : 8; // Increase force when boost is active
  let TURN = boostActive ? 1 : 0.6;
  const maxSpeed = 200;
  ...
  useFrame((state, delta) => {
    ...
    if (isFirstPerson) {
      const carQuaternion = rigidBody.current.rotation(); // Get the car's quaternion (rotation)
        const firstPersonOffset = new THREE.Vector3(-0.5, -0.95, 0); // Adjust offset as needed
